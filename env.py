...
class Environment(object):
    ...
    def reset(self):
        ...
        self.user_location = np.array(
            self.user_location).reshape(self.user_num, 2)
        ...
        # 初始化无人机的位置
        # 无人机x位置随机初始化为(0，100)之间
        uav_location_x = 0
        # 无人机y位置随机初始化为(0，100)之间
        uav_location_y = 0
        for i in range(self.user_num):
            self.uav_location[i][0] = uav_location_x
            self.uav_location[i][1] = uav_location_y

        self.uav_location = np.array(
            self.uav_location).reshape(self.user_num, 2)
    ...
    def step(self, action, obs):
        ...
        if x > x_size - 5:
            if math.cos(action) > (100 - x) / 5:
                ...
                reward, si_best, delay_power_sum, delay_power_by_user = pdpso(
                    obs, x, y)
        ...
        min_max_scaler = preprocessing.MinMaxScaler(
            feature_range=(0, 1))  # 归一化函数
        next_obs_normalization[:, 0:3] = min_max_scaler.fit_transform(
            next_obs[:, 0:3])
        ...
        done = (x < 0 or x > x_size) or (y < 0 or y > y_size) or (
            x > x_size - 5 and y > y_size - 5)
        # done = x > x_size and y > y_size

        return next_obs, next_obs_normalization, reward, si_best, done, x, y, delay_power_sum, delay_power_by_user
