...
def run_episode(agent, env, rpm):
    ...
    while True:
        ...
        next_obs, next_obs_normalization, reward, si_best, done, x, y, delay_power_sum, delay_power_by_user = env.step(
            action, obs)
        uav_path_x.append(x)
        uav_path_y.append(y)
        ...
        action = [action]
        # 方便存入replaymemory
        rpm.append((obs_normalization, action, REWARD_SCALE *
                   reward, next_obs_normalization, done))
    ...
    return total_reward, total_loss, si_best, step_sum, uav_path_x, uav_path_y, obs, delay_power_sum, delay_power_by_user
...
def main():
    ...
    while len(rpm) < MEMORY_WARMUP_SIZE:
        ...
        i = i + 1
        logger.info('pool de xp {}. {}/{}'.format(i,
                    len(rpm), MEMORY_WARMUP_SIZE))
    ...
    while episode < max_episode:
        ...
        for i in range(step_num):
            ...
            user_location_y = []
            user_data = []
            total_reward, total_loss, si_best, step_sum, uav_path_x, uav_path_y, obs, delay_power_sum, delay_power_by_user = run_episode(
                agent, env, rpm)
            # 平均reward
            ...
            if episode == max_episode - 1:
                result_sum.append(delay_power_sum)
                result_by_user.append(delay_power_by_user)
                ...
                for j in range(user_num):
                    logger.info('si_best.size:{} j:{} user_num:{}'.format(
                        len(si_best), j, user_num))
                    if len(si_best) != user_num:
                        logger.info('BREAK')
                        break
                    if si_best[j] == 0:
                        ...
                        ax.scatter3D(user_location_x_local,
                                     user_location_y_local, 0, s=20., c='b')
                    elif si_best[j] == 1:
                        ...
                        ax.scatter3D(user_location_x_load,
                                     user_location_y_load, 0, s=20., c='r')

                ax.plot3D(uav_trajectory_x, uav_trajectory_y, 100, 'green')
                fig.savefig('figure/user-uav/figure%i.png' % i)
    ...
    df.to_csv('result_sum.csv', index=False)

    df = pd.DataFrame(result_by_user)
    df.to_csv('result_by_user.csv', index=False)
