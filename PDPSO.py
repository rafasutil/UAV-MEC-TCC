...
def pdpso(obs, x, y):
    ...
    # 初始化pbest、gbest
    Pro_load_start, Pro_local_start, _, _ = env.System_Model(
        user_location, L, uav_location)
    # Pro_load_start = min_max_scaler.fit_transform(Pro_load_start)
    # Pro_local_start = min_max_scaler.fit_transform(Pro_local_start)

    pbest = np.append(Pro_local_start, Pro_local_start,
                      axis=1)     # 初始化pbest为初始粒子位置
    gbest = np.append(Pro_load_start, Pro_load_start,
                      axis=1)       # 初始化pbest为全本地总能耗和时延加权和
    ...
    while epoch < iterations:
        ...
        Pro = []                                                    # 问题定义Problem
        particle_V = []                                             # 粒子速度
        Pro_load, Pro_local, delay_power_sum, delay_power_by_user = env.System_Model(
            particel_x, L, uav_location)
        ...
        for i in range(particle):
            ...
            particle_V_ = math.sqrt(
                particle_v[i][0] ** 2 + particle_v[i][1] ** 2)
        ...
        # 将[[],[],[]]数组降维至[, , ,]
        Pro = [token for st in Pro for token in st]
        Pro_sum_ = sum(Pro)
        ...
        for i in range(user_num):
            for j in range(user_num):
                if i != j:
                    d[i][j] = pow((particel_x[i][0] - particel_x[j][0]) **
                                  2 + (particel_x[i][1] - particel_x[j][1]) ** 2, 1/2)
                else:
                    d[i][j] = 0
        ...
        # mask = d != 0
        # d_new = d[mask]
        d_new = np.array(d_new).reshape(
            user_num, user_num-1)           # 去掉自身距离的新矩阵d_new

        # 每个用户与其他用户中的最小距离d_min
        d_min_ = np.min(d_new, axis=1)
        # 每个用户与其他用户距离的平均值
        d_average_ = np.mean(d_new, axis=1)
        d_min.append(d_min_)
    ...
    return result, si_best, delay_power_sum, delay_power_by_user
